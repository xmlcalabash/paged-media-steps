...
import com.xmlcalabash.util.{MediaType, PipelineOutputConsumer, PipelineOutputDocument}
import org.scalatest.flatspec.AnyFlatSpec

class FoTest extends AnyFlatSpec {
  val PDF = MediaType.parse("application/pdf")

  "Running fo.xpl " should " produce a PDF" in {
    // Frustratingly, these don't run in CI.
    if (sys.env.contains("CIRCLECI")) {
      System.err.println("Skipping FO test on CI host")
    } else {
      val calabash = XMLCalabash.newInstance()
      val pdf = new Consumer()
      val result = new PipelineOutputConsumer("result", pdf)
      calabash.args.parse(List("src/test/resources/fo.xpl"))
